...
// Slack (from procore-issue and safety-broadcast messages) in response to a
// natural-language question. Scoped to the requesting user's own access via
// their user token, same as the existing Slack MCP connection in agent.js.
...
export async function searchWorkspace(query, userToken) {
  ...
  throw new Error('Not implemented: wire up lib/rtsEngine.js#searchWorkspace');
}
