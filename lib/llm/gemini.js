 ...
 * @param {string} [params.model] - Override the model for this call (defaults to GEMINI_MODEL / flash-lite).
 *   Lets a tool-heavy caller (e.g. the DM agent) use a stronger model without changing the global default.
 * @returns {Promise<{ responseText: string, history: import('@google/genai').Content[] }>}
 */
export async function runLlmTurn({ systemPrompt, history, text, tools = [], mcpServers = [], model }) {
