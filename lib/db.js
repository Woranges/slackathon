...
/**
 * Find the most recent broadcast for a site — used to route an inbound worker
 * acknowledgment ("got it") to the broadcast it's answering, since the SMS
 * itself carries no broadcast id. Returns null when the site has no broadcasts.
 * @param {string} siteId
 * @returns {Promise<Broadcast | null>}
 */
export async function getLatestBroadcastForSite(siteId) {
  /** @type {Broadcast | null} */
  let latest = null;
  for (const b of BROADCASTS.values()) {
    if (b.siteId !== siteId) continue;
