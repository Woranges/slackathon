...
// Acks and escalations both store an ISO instant, not just membership: "who was
// warned, and when" is the whole point of the audit trail — a bare Set could
// prove a worker acknowledged but never prove when, which is exactly what a
// dispute turns on. Keyed `${broadcastId}::${workerPhone}` so each counts once.
/** @type {Map<string, string>} broadcastId::phone -> ISO instant of the ack. */
const BROADCAST_ACKS = new Map();
/** @type {Map<string, string>} broadcastId::phone -> ISO instant of the escalation call. */
const BROADCAST_ESCALATIONS = new Map();
 ...
 * @returns {string}
 */
function ackKey(broadcastId, workerPhone) {
  return `${broadcastId}::${workerPhone}`;
}
...
/**
 * Record that a worker acknowledged a broadcast, with the instant they did it.
 * Idempotent, and the FIRST acknowledgment wins: the legally relevant instant is
 * when the worker first confirmed they were warned, so a duplicate "ok" must not
 * quietly rewrite the record to a later time.
 * @param {string} broadcastId
 * @param {string} workerPhone
 * @returns {Promise<void>}
 */
export async function recordBroadcastAck(broadcastId, workerPhone) {
  const key = ackKey(broadcastId, workerPhone);
  if (BROADCAST_ACKS.has(key)) return;
  BROADCAST_ACKS.set(key, new Date().toISOString());
}

/**
 * Record that a worker was escalated to an automated voice call, with the
 * instant the call was placed. This is the "proof of due diligence" half of the
 * audit trail: we texted, they didn't reply, so we called them at 11:47pm.
 * @param {string} broadcastId
 * @param {string} workerPhone
 * @returns {Promise<void>}
 */
export async function recordEscalation(broadcastId, workerPhone) {
  const key = ackKey(broadcastId, workerPhone);
  if (BROADCAST_ESCALATIONS.has(key)) return;
  BROADCAST_ESCALATIONS.set(key, new Date().toISOString());
}

/**
 * @typedef {Object} BroadcastAuditRow
 * @property {string} phone
 * @property {string} name
 * @property {string} language - The worker's ISO 639-1 language at the time of the broadcast.
 * @property {string | null} ackedAt - ISO instant they acknowledged, or null.
 * @property {string | null} escalatedAt - ISO instant they were voice-called, or null.
 */

/**
 * The full audit trail for a broadcast: one row per worker it was sent to,
 * showing whether and when they acknowledged, and whether and when they were
 * escalated to a voice call. Powers the named scoreboard and the audit summary
 * posted to Slack.
 * @param {string} broadcastId
 * @returns {Promise<BroadcastAuditRow[]>}
 */
export async function getBroadcastAudit(broadcastId) {
  const broadcast = BROADCASTS.get(broadcastId);
  if (!broadcast) return [];

  const workers = await getWorkersBySite(broadcast.siteId);
  return workers.map((worker) => ({
    phone: worker.phone,
    name: worker.name ?? worker.phone,
    language: worker.preferredLanguage,
    ackedAt: BROADCAST_ACKS.get(ackKey(broadcastId, worker.phone)) ?? null,
    escalatedAt: BROADCAST_ESCALATIONS.get(ackKey(broadcastId, worker.phone)) ?? null,
  }));
}
...
export async function getAckStatus(broadcastId) {
  ...
  let acknowledged = 0;
  for (const key of BROADCAST_ACKS.keys()) {
